...
main() {
  ...
  case "${1:-help}" in
    ...
    data|data-sync|data-gen|generate-data|gen-data)
      # Professional Data Toolkit - all data commands go to auto_app_data generation/
      shift || true
      data_toolkit="$ROOT_DIR/auto_app_data generation/toolkit"
      data_automate="$ROOT_DIR/auto_app_data generation/automate.sh"
      
      if [[ -f "$data_toolkit" ]]; then
        echo ""
        echo "🔧 Using: auto_app_data generation/toolkit"
        echo ""
        bash "$data_toolkit" "$@"
      elif [[ -f "$data_automate" ]]; then
        echo ""
        echo "🔧 Using: auto_app_data generation/automate.sh"
        echo ""
        bash "$data_automate" "$@"
      else
        echo ""
        echo "╔═══════════════════════════════════════════════════════════════════════════╗"
        echo "║  📦 Data Toolkit Not Found                                                ║"
        echo "╚═══════════════════════════════════════════════════════════════════════════╝"
        echo ""
        echo "Expected location:"
        echo "  auto_app_data generation/"
        echo ""
        echo "This folder should contain:"
        echo "  • toolkit           - Main command interface"
        echo "  • automate.sh       - Automation scripts"
        echo "  • config/           - Project configurations"
        echo "  • projects/         - Per-project settings"
        echo "  • core/             - Core libraries"
        echo ""
        exit 1
      fi
      ;;
    ...
    data-toolkit|toolkit)
      # DNS Data Toolkit Generator - generates auto_app_data generation interface
      shift || true
      toolkit_generator="$CORE_DIR/generators/data_toolkit_generator.sh"
      if [[ -f "$toolkit_generator" ]]; then
        bash "$toolkit_generator" "$@"
      else
        log_error "Data toolkit generator not found" "TOOLKIT"
        echo "❌ Data toolkit generator not found at: $toolkit_generator"
        exit 1
      fi
      ;;
...
EOF
    ...
    *)
      ...
      echo ""
      echo "Usage: $0 {auto|basic|heavy|smart|analyze|optimize|review|refactor|check|qa|fix|test|health|cache|metrics|log|config|init|tips|templates|package|data-sync} [arguments]"
      echo ""
      ...
      echo ""
      echo "📊 Data Toolkit (auto_app_data generation/):"
      echo "  data --project NAME config    - Show project configuration"
      echo "  data --project NAME validate  - Validate configuration"
      echo "  data --project NAME setup     - Initial GitHub repo setup"
      echo "  data --project NAME upload    - Upload data to GitHub"
      echo "  data --project NAME sync      - Full sync workflow"
      echo "  data --project NAME verify    - Verify data integrity"
      echo "  data projects                 - List available projects"
      echo ""
      echo "🛠️  Data Toolkit Generator:"
      echo "  data-toolkit generate       - 🆕 Generate auto_app_data interface"
      echo "  data-toolkit info           - 🆕 Show what will be generated"
      echo "  data-toolkit --help         - 🆕 Show toolkit generator help"
      echo ""
      ...
      echo "  ✅ 🆕 Universal data synchronization system (GitHub integration)"
      echo ""
      ...
      echo ""
      echo "📊 Data Toolkit Quick Start:"
      echo "  1. bash .cursorrules data projects                  # List projects"
      echo "  2. bash .cursorrules data --project NAME config     # Show config"
      echo "  3. bash .cursorrules data --project NAME sync       # Full sync"
      echo ""
      echo "  Or use directly: cd 'auto_app_data generation' && ./toolkit --help"
      echo ""
