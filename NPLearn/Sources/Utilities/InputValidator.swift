...
struct InputValidator {
    ...
    // MARK: - Nepali Text Validation
    
    static func containsNepaliScript(_ text: String) -> Bool {
        let nepaliRegex = #"[\u0900-\u097F]+"#
        let nepaliTest = NSPredicate(format: "SELF MATCHES %@", nepaliRegex)
        return nepaliTest.evaluate(with: text)
    }
}
