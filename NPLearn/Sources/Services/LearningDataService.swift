...
//  JSON-Driven Architecture matching JPLearning
//
...
final class LearningDataService: ObservableObject {
    ...
    func loadLearningData() async {
        ...
        let filename = "nepali_learning_data_\(level.rawValue.lowercased())"
        ...
        do {
            if let data = result.0 {
                ...
                // Generate derived data in background
                ...
                AppLogger.info("   About to assign: Flashcards: \(parsed.flashcards.count), Grammar: \(parsed.grammar.count)")
                ...
                AppLogger.info("   After assignment: Flashcards: \(self.flashcards.count), Grammar: \(self.grammarPoints.count)")
                ...
                AppLogger.info("   - Practice Questions: \(self.practiceQuestions.count)")
    ...
    func setLevel(_ level: LearningLevel) async {
        ...
        await loadLearningData()
        AppLogger.info("✅ [SET LEVEL] Finished loading \(level.rawValue) - Flashcards: \(flashcards.count), Grammar: \(grammarPoints.count)")
